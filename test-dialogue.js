...
const { getAgentResponse } = require('./lib/agent');
const { createInitialState } = require('./lib/trainer-state');
...
async function testDialogue(scenarioId, difficulty, userMessages) {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`Testing: ${scenarios[scenarioId].title} (${difficulty} mode)`);
  console.log(`${'='.repeat(60)}\n`);

  let state = createInitialState(scenarioId, scenarios[scenarioId].must_cover_high5);
